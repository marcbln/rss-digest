#!/bin/bash
# Workflow: Generate digest and publish to GitHub Pages blog
# Usage: ./scripts/generate-and-blog.sh <config-name> [--dry-run]
#
# Example: ./scripts/generate-and-blog.sh ai-assisted-programming
...
if [ "$2" = "--dry-run" ]; then
    DRY_RUN=true
    echo "[DRY RUN] Would generate and publish to blog: $CONFIG_NAME"
fi
...
# Check if generation succeeded (checking for success: true in the last JSON line)
...
echo ""
echo "=== Step 2: Publishing to Blog (GitHub Pages) ==="

if [ "$DRY_RUN" = true ]; then
    echo "[DRY RUN] Would publish: $DIGEST_FILE"
    echo "[DRY RUN] Command: rss-digest publish github-pages $DIGEST_FILE"
    echo ""
    echo "Dry run complete. Digest file exists at: $DIGEST_FILE"
else
    $RSS_DIGEST publish github-pages "$DIGEST_FILE"
    echo ""
    echo "✓ Workflow complete! Digest generated and published to blog."
fi
